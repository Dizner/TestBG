...
//            Log.d("图片加载", e == null ? "失败" : e.getMessage() + "失败");
            return false;
        }

        @Override
        public boolean onResourceReady(Object resource, Object model, Target target, boolean isFromMemoryCache, boolean isFirstResource) {
//            Log.d("图片加载--onResourceReady", model.toString() + "是否内存缓存：" + isFromMemoryCache + "是否第一次：" + isFirstResource);
            return false;
        }
    };
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).listener(listener).crossFade().into(mImageView);
    }
